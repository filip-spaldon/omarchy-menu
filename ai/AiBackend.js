...
var configuredConfig = runtimeConfig // what ai.json / the Omarchy default chose

// The config a question to `agentId` runs with: its model is the explicit
// top-level "model" when asking the configured agent, else that agent's
// entry in "models"; its effort comes from "efforts". resumeModel is the
// explicit model only, so the terminal continuation is not pinned to the
// cheap launcher default.
function resolveFor(base, agentId) {
  var next = {}
  for (var k in base) next[k] = base[k]
  next.agent = agentId
  var explicit = (agentId === base.agent && base.model) ? base.model : null
  next.model = explicit || (base.models && base.models[agentId]) || null
  next.effort = (base.efforts && base.efforts[agentId]) || null
  next.resumeModel = explicit
  return next
}

function loadConfig(rawText, omarchyAgent) {
  var result = AiConfig.mergeConfig(rawText, omarchyAgent)
  configuredConfig = result.config
  runtimeConfig = resolveFor(result.config, result.config.agent)
  return result
}

// Local addition (omarchy-menu-omni): the agent can be switched per question
// from the launcher, since the configured one is not always the one with
// usage left. The config object is replaced, never mutated, so a running
// generation keeps the config it was started with. A model pinned in ai.json
// belongs to the configured agent and is dropped for any other.
function setAgent(agentId) {
  var adapter = AiAdapters.get(agentId)
  if (!adapter || adapter.disabledReason) return false
  runtimeConfig = resolveFor(configuredConfig, agentId)
  return true
}

// Adapters that may be offered at all (installed or not): every one except
// those disabled for headless use.
function selectableAgents() {
  ...
  for (var i = 0; i < AiConfig.SUPPORTED_AGENTS.length; i++) {
    ...
    if (adapter && !adapter.disabledReason) out.push({ id: adapter.id, label: adapter.label, binary: adapter.binary })
  }
  return out
}
...
function agentDisplay() {
  ...
  return {
    ...
    effortLabel: runtimeConfig.effort || null,
    supported: adapter !== null && !adapter.disabledReason
  }
}
  ...
  if (!adapter) {
    ...
    var errorMsg = isKnownOmarchy
      ? runtimeConfig.agent + " does not yet support headless AI mode. " +
        ...
        "(or set \"agent\" in ~/.local/state/omarchy-menu-omni/ai.json)."
...
function handleExit(gen, exitCode) {
  ...
  if (exitCode !== 0 || producedNothing) {
    ...
    if (!classification) {
      ...
      if (session.stderrText) console.warn("[omarchy-menu-omni/ai] unclassified failure, raw stderr: " + session.stderrText)
      classification = {
...
function snapshot() {
  ...
  return {
    ...
    effortLabel: (session.config && session.config.effort) || null,
    supported: true,
